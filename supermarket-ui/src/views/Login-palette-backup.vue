<template>
  ...
  <div class="landing-wrapper" ref="scrollContainer" @scroll="handleScroll">
    ...
    <!-- 调色盘 -->
    <div class="color-palette" :class="{ open: paletteOpen }">
      <button class="palette-toggle" @click="paletteOpen = !paletteOpen">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><path d="M12 2a10 10 0 0 1 0 20"/><circle cx="12" cy="12" r="3"/></svg>
      </button>
      <div class="palette-body" v-show="paletteOpen">
        <div class="palette-row"><label>背景</label><input type="color" v-model="palBg" @input="applyPalette" /></div>
        <div class="palette-row"><label>点缀</label><input type="color" v-model="palAccent" @input="applyPalette" /></div>
        <div class="palette-row"><label>标题</label><input type="color" v-model="palTitle" @input="applyPalette" /></div>
        <div class="palette-row"><label>文字</label><input type="color" v-model="palText" @input="applyPalette" /></div>
        <div class="palette-presets">
          <button v-for="p in presets" :key="p.name" @click="usePreset(p)" :title="p.name">{{ p.name }}</button>
        </div>
      </div>
    </div>
  </div>
...
const palBg = ref('#1B579C')
const palAccent = ref('#508BC3')
const palTitle = ref('#D0E2F2')
const palText = ref('#B0CFE5')
const presets = [
  { name: 'Capri', bg: '#1B579C', accent: '#508BC3', title: '#D0E2F2', text: '#B0CFE5' },
  { name: 'Cerulean', bg: '#28446C', accent: '#6BA5D8', title: '#C8DCF0', text: '#A8C8E0' },
  { name: 'Deep Navy', bg: '#091420', accent: '#508BC3', title: '#B4D0E8', text: '#90B8D5' },
  { name: 'Ocean', bg: '#0D2B45', accent: '#5BA0D0', title: '#C5DDF2', text: '#A0C5E0' },
  { name: 'Slate', bg: '#1E2D3D', accent: '#7098B8', title: '#D5E5F0', text: '#B0C8D8' },
  { name: 'Midnight', bg: '#0A1628', accent: '#4A8EC0', title: '#C0D8F0', text: '#98BBD5' },
]

const applyPalette = () => {
  const bg = palBg.value, accent = palAccent.value
  // 背景
  document.querySelectorAll('.landing-wrapper, .hero-section, .feature-section, .feature-section.alt, .modules-section, .intro-overlay, .minimal-header.scrolled').forEach(el => {
    el.style.background = bg
    el.style.backgroundColor = bg
  })
  // 导航滚动背景（带透明度）
  const bgR = parseInt(bg.slice(1,3), 16), bgG = parseInt(bg.slice(3,5), 16), bgB = parseInt(bg.slice(5,7), 16)
  document.querySelectorAll('.minimal-header').forEach(el => {
    el.style.setProperty('--pal-bg-rgb', `${bgR},${bgG},${bgB}`)
  })
  // 滚动指示器
  document.querySelectorAll('.hint-dot, .hint-line, .hint-text').forEach(el => {
    el.style.setProperty('--pal-bg', bg)
  })
  // 卡片/框架
  document.querySelectorAll('.media-frame, .mod-card').forEach(el => {
    el.style.background = `rgba(${bgR},${bgG},${bgB},0.5)`
    el.style.borderColor = `rgba(255,255,255,0.08)`
  })
  // 点缀色
  document.querySelectorAll('.logo-dot, .accent-dot, .section-tag, .li-dot, .tech-link').forEach(el => {
    el.style.color = accent
  })
  // 海浪颜色
  const waveColors = [
    `rgba(${Math.min(bgR+120,255)},${Math.min(bgG+120,255)},${Math.min(bgB+100,255)},0.15)`,
    `rgba(${Math.min(bgR+100,255)},${Math.min(bgG+100,255)},${Math.min(bgB+80,255)},0.1)`,
    `rgba(${Math.min(bgR+80,255)},${Math.min(bgG+80,255)},${Math.min(bgB+60,255)},0.07)`,
  ]
  // 更新 Canvas 海浪（需要重建）
  if (ctx && animFrame) {
  ...
  document.querySelectorAll('.cursor-ring').forEach(el => {
    el.style.background = `radial-gradient(circle, rgba(${Math.min(bgR+100,255)},${Math.min(bgG+100,255)},${Math.min(bgB+80,255)},0.15) 0%, rgba(${Math.min(bgR+60,255)},${Math.min(bgG+60,255)},${Math.min(bgB+50,255)},0.05) 40%, transparent 70%)`
    el.style.borderColor = `rgba(${Math.min(bgR+100,255)},${Math.min(bgG+100,255)},${Math.min(bgB+80,255)},0.12)`
  })
}

const usePreset = (p) => {
  palBg.value = p.bg; palAccent.value = p.accent; palTitle.value = p.title; palText.value = p.text
  applyPalette()
}
...
const initWaves = () => {
  ...
  const layers = [
    { amp: 30, freq: 0.008, speed: 0.3, color: 'rgba(200,225,245,0.15)', height: 0.3, lineWidth: 1.5 },
    { amp: 20, freq: 0.012, speed: 0.5, color: 'rgba(180,215,235,0.1)', height: 0.25, lineWidth: 1 },
    { amp: 15, freq: 0.018, speed: 0.7, color: 'rgba(160,200,225,0.07)', height: 0.2, lineWidth: 0.8 },
  ]
...
.intro-overlay {
  ...
  background: #1B579C; display: flex; align-items: center; justify-content: center;
}
.intro-slide {
...
.landing-wrapper {
  ...
  background: #1B579C; color: rgba(200,220,240,0.55); cursor: none;
}
.section {
...
.hero-section {
  ...
  background: #1B579C; padding-left: 8vw;
}
...
/* ================== 特性展示屏 — 统一暗调背景 ================== */
.feature-section {
  background: #040b1a; color: rgba(155,200,225,0.6); justify-content: center;
}
.feature-section.alt { background: #1B579C; }
...
/* ================== 模块屏 ================== */
.modules-section { background: #040b1a; justify-content: center; align-items: center; padding: 80px 40px; }
